...
Result ICommunicationService::Connect(const ConnectNetworkData &dat, const NetworkInfo& data) {
    ...
    bool connected_ok = WaitForResponse(ryu_ldn::protocol::PacketId::Connected, 8000);

    if (!connected_ok && !m_use_p2p_proxy) {
        ...
        if (send_result == ryu_ldn::network::ClientOpResult::Success) {
            connected_ok = WaitForResponse(ryu_ldn::protocol::PacketId::Connected, 8000);
        }
    }
...
Result ICommunicationService::ConnectPrivate(const ConnectPrivateData &data) {
    ...
    constexpr uint64_t response_timeout_ms = 8000; // FailureTimeout in Ryujinx
    bool connected_ok = WaitForResponse(ryu_ldn::protocol::PacketId::Connected, response_timeout_ms);
    ...
    if (!connected_ok && !m_use_p2p_proxy) {
        ...
        if (send_result == ryu_ldn::network::ClientOpResult::Success) {
            connected_ok = WaitForResponse(ryu_ldn::protocol::PacketId::Connected, response_timeout_ms);
        }
    }
