/**
 * @file ibsd_mitm_service.hpp
 * @brief BSD MITM Service interface definition (bsd:u)
 *
 * This file defines the IPC interface for the BSD socket MITM service.
 * We intercept bsd:u to detect and proxy LDN traffic through the RyuLdn server.
 *
 * ## Strategy
 *
 ...
 *
 * This macro defines all the IPC commands we intercept from the BSD service.
 * We use AMS_SF_DEFINE_MITM_INTERFACE to create a MITM-capable interface.
 *
 * Key intercepted commands:
 * - Socket: Track new sockets
 * - Bind: Detect LDN address binding
 * - Connect: Detect LDN address connections
 * - Send/SendTo: Proxy outgoing data
 * - Recv/RecvFrom: Proxy incoming data
 * - Close: Clean up tracked sockets
 */
