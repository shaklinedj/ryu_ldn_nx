...
; Server port
port = 30456
; Use TLS encryption (0/1) — NOT IMPLEMENTED: no TLS code exists, always plain TCP
use_tls = 0
...
passphrase =
; Network interface (empty = auto) — NOT YET WIRED: always auto-detects
interface =
...
| `use_tls` | `true` | ⚠️ **Not implemented** — no TLS code exists, always plain TCP regardless of this setting |
...
| `interface` | _(empty)_ | ⚠️ Parsed but **not used** — always auto-detects network interface |
...
### `[perf]` — Performance Tuning (Experimental, Not Yet Wired)

> **Warning**: The `[perf]` section is defined in the example config file but **not yet parsed or used** by the sysmodule. These keys are silently ignored. They are included for forward compatibility — once wired, they will take effect automatically.
...
Changing `[perf]` values currently has **no effect**. These options will become functional in a future release.
