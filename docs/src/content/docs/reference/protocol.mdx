...
LDN IPs in `NetworkInfo`, `ProxyConfig`, and `m_ipv4_address` are all in **Ryujinx format**: big-endian read as `uint32` (e.g., `10.114.0.1` = `0x0A720001`). BSD `sockaddr_in.sin_addr` uses **network byte order** (which is also big-endian). `GetAddr()` does `bswap32`, `sin_addr` is stored in network byte order; `bswap32` converts Ryujinx format → network byte order (so `GetAddr()`, which does another `bswap32`, returns the original Ryujinx-format value). **Never double-convert.**
